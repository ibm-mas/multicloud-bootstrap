  ...
  rosa_cm=$(oc get cm rosa-brand-logo -n openshift-config | awk  'NR==2 {print $2 }')
    if [[ $rosa_cm -eq 1 ]]; then
      ...
      if [[ $currentOpenshiftVersion =~ ${op_versions[rosaVersion415]} ]]; then
          ...
          log " Unsupported ROSA version $currentOpenshiftVersion. Supported ROSA versions are 4.15.x"
        export SERVICE_NAME=" Unsupported ROSA version $currentOpenshiftVersion. Supported ROSA versions are 4.15.x"
        SCRIPT_STATUS=29
...
function getOCPVersion() {
  ...
  log " OCP version is $currentOpenshiftVersion"
    ...
    #fi
      ...
      log " Unsupported Openshift version $currentOpenshiftVersion. Supported OpenShift version is 4.15.x"
    export SERVICE_NAME=" Unsupported Openshift version $currentOpenshiftVersion. Supported OpenShift versions is 4.15.x"
    SCRIPT_STATUS=29
    return $SCRIPT_STATUS
   fi
}
