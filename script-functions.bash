...
op_versions['ocpVersion416']='^4\.([1][6])?(\.[0-9][0-9]+.*)*$'
op_versions['rosaVersion415']='^4\.([1][5])?(\.[0-9]+.*)*$'
op_versions['rosaVersion416']='^4\.([1][6])?(\.[0-9]+.*)*$'
...
checkROSA(){
  rosa_cm=$(oc get cm rosa-brand-logo -n openshift-config | awk  'NR==2 {print $2 }')
    if [[ $rosa_cm -eq 1 ]]; then
      ...
      if [[ $currentOpenshiftVersion =~ ${op_versions[rosaVersion415]} ]]; then
          log " ROSA Cluster Supported Version"
        elif [[ $currentOpenshiftVersion =~ ${op_versions[rosaVersion416]} ]]; then
          log " ROSA Cluster Supported Version"
        else
          log " Unsupported ROSA version $currentOpenshiftVersion. Supported ROSA versions are 4.15.x and 4.16.x"
        export SERVICE_NAME=" Unsupported ROSA version $currentOpenshiftVersion. Supported ROSA versions are 4.15.x and 4.16.x"
        SCRIPT_STATUS=29
...
function getOCPVersion() {
  currentOpenshiftVersion=$(oc get clusterversion | awk  'NR==2 {print $2 }')
  log " OCP version is $currentOpenshiftVersion"
    if [[ ${currentOpenshiftVersion} =~ ${op_versions[ocpVersion415]} ]]; then
      log " OCP Supported Version"
    elif [[ ${currentOpenshiftVersion} =~ ${op_versions[ocpVersion415]} ]]; then
      log " OCP Supported Version"
    else
      log " Unsupported OCP version $currentOpenshiftVersion. Supported OCP versions are 4.15.x and 4.15.x"
    export SERVICE_NAME=" Unsupported OCP version $currentOpenshiftVersion. Supported OCP versions are 4.15.x and 4.16.x"
    SCRIPT_STATUS=29
    return $SCRIPT_STATUS
    fi
}
...
function getEFS() {
  ...
  check_for_csv_success=$(oc get csv  --all-namespaces | awk -v pattern="$1" '$2 ~ pattern  { print }'  | awk -F' ' '{print $NF}')
  sc_name=$(oc get sc | grep $CPD_PRIMARY_STORAGE_CLASS | awk -F' ' '{print $1}')
