...
if [[ $MONGO_FLAVOR == "Amazon DocumentDB" ]]; then
    ...
    log "docdb-create-vpc-peer.sh .......... starts"
    log "docdb-create-vpc-peer.sh : REQUESTER_VPC_ID : $REQUESTER_VPC_ID" #BOOTNODE_VPC_ID or VPC_ID
    log "docdb-create-vpc-peer.sh : ACCEPTER_VPC_ID : $ACCEPTER_VPC_ID" #DOCUMENTDB_VPC_ID
	log "docdb-create-vpc-peer.sh : ACCEPTER_REGION : $ACCEPTER_REGION"
    if [[ "${VPC_1}" != "${VPC_2}" ]]; then

        log "docdb-create-vpc-peer.sh : Invoke docdb-create-iam-user.sh"

		sh $GIT_REPO_HOME/mongo/docdb/docdb-create-iam-user.sh
		if [ $? -ne 0 ]; then
			SCRIPT_STATUS=36
			exit $SCRIPT_STATUS
		fi
		...
		if [[ -z "$_VPC_2" ]]; then
			SCRIPT_STATUS="39"
			log "docdb-create-vpc-peer.sh : User entered $_VPC_2 is not found in region $ACCEPTER_REGION, exiting..."
			exit $SCRIPT_STATUS
		fi
		...
		if [[ -z "$_VPC_1" ]]; then
			SCRIPT_STATUS="40"
			log "docdb-create-vpc-peer.sh : $_VPC_1 is not found in region $ACCEPTER_REGION, exiting..."
			exit $SCRIPT_STATUS
		fi
		...
		if [[ -z "$VPC_1_CIDR" || -z "$VPC_2_CIDR" ]]; then
			SCRIPT_STATUS=35
			log "docdb-create-vpc-peer.sh : VPC_1_CIDR or VPC_2_CIDR is empty, exiting..."
			exit $SCRIPT_STATUS
		fi

        log "docdb-create-vpc-peer.sh : Invoke create-vpc-peering-connection"
        ...
        sleep 30
        log "docdb-create-vpc-peer.sh : VPC_PEERING_CONNECTION_ID=$VPC_PEERING_CONNECTION_ID"
        if [[ -z "$VPC_PEERING_CONNECTION_ID" ]]; then
            SCRIPT_STATUS=35
            log "docdb-create-vpc-peer.sh : VPC_PEERING_CONNECTION_ID is empty, exiting..."
			exit $SCRIPT_STATUS
        fi
        if [[ -n "$VPC_PEERING_CONNECTION_ID" ]]; then
            log "docdb-create-vpc-peer.sh : aws ec2 accept-vpc-peering-connection"
			...
			while [[ $found == "false" ]] && [[ $counter < 20 ]]; do
				...
				if [[ $? -ne 0 ]]; then
					SCRIPT_STATUS=35
					log "docdb-create-vpc-peer.sh : ACCEPT_VPC_PEERING_CONNECTION failed, exiting..."
					exit $SCRIPT_STATUS
				else
					log "docdb-create-vpc-peer.sh : ACCEPT_VPC_PEERING_CONNECTION accepted, waiting for provisioning and status change to active..."
					sleep 60
					...
					log "docdb-create-vpc-peer.sh : ACCEPT_VPC_PEERING_CONNECTION==$ACCEPT_VPC_PEERING_CONNECTION"
			...
			if [[ -z "$ACCEPT_VPC_PEERING_CONNECTION"  ]]; then
				SCRIPT_STATUS=35
				log "docdb-create-vpc-peer.sh : ACCEPT_VPC_PEERING_CONNECTION is empty, exiting..."
				exit $SCRIPT_STATUS
			fi

			# Creating Routes for vpc peering at cluster and mirror machines
			log "docdb-create-vpc-peer.sh : create routes started"
			...
			if [[ -n "$VPC_PEERING_CONNECTION_ID" && "${ACCEPT_VPC_PEERING_CONNECTION}" == *"active"* ]] ;then
				...
				if [[ -n $RTS ]]; then
					...
					for VPC_1_ROUTE_TABLE_ID in $RTS; do
						...
						--query RouteTables[].RouteTableId  --output=text`
						log "docdb-create-vpc-peer.sh : EXISTING_ROUTE_TABLE_ID=$EXISTING_ROUTE_TABLE_ID"
						if [[ -z $EXISTING_ROUTE_TABLE_ID ]]; then
							log "docdb-create-vpc-peer.sh : aws ec2 create-route $VPC_1_ROUTE_TABLE_ID $VPC_2_CIDR $VPC_PEERING_CONNECTION_ID started"
							...
							sleep 10
							log "docdb-create-vpc-peer.sh : CREATE_ROUTE_1=$CREATE_ROUTE_1"					
							if [[ ("${CREATE_ROUTE_1}" != "True") ]]; then
								...
								SCRIPT_STATUS=37
								log "docdb-create-vpc-peer.sh : aws ec2 create-route CREATE_ROUTE_1 creation failed, exiting..."
								exit $SCRIPT_STATUS
							fi						
						fi
					done
				...
				if [[ -n $RTS ]]; then
					...
					for VPC_2_ROUTE_TABLE_ID in $RTS; do
						...
						log "docdb-create-vpc-peer.sh : EXISTING_ROUTE_TABLE_ID_2=$EXISTING_ROUTE_TABLE_ID"						
						if [[ -z $EXISTING_ROUTE_TABLE_ID ]]; then
							log "docdb-create-vpc-peer.sh : aws ec2 create-route $VPC_2_ROUTE_TABLE_ID $VPC_1_CIDR $VPC_PEERING_CONNECTION_ID started"
							...
							sleep 10      
							log "docdb-create-vpc-peer.sh : CREATE_ROUTE_2=$CREATE_ROUTE_2"
							if [[ ("${CREATE_ROUTE_2}" != "True") ]]; then
								SCRIPT_STATUS=37
								log "docdb-create-vpc-peer.sh : aws ec2 create-route CREATE_ROUTE_2 creation failed, exiting..."
								exit $SCRIPT_STATUS
							fi					
						fi
				...
				SCRIPT_STATUS=37
				log "docdb-create-vpc-peer.sh : route not created, VPC_PEERING_CONNECTION_ID=$VPC_PEERING_CONNECTION_ID : ACCEPT_VPC_PEERING_CONNECTION=$ACCEPT_VPC_PEERING_CONNECTION"
				exit $SCRIPT_STATUS
			fi
			log "docdb-create-vpc-peer.sh : create routes completed"
        fi
    fi
