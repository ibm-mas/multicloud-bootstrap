...
mark_provisioning_failed() {
  ...
  if [[ $retcode -eq 2 ]]; then
    ...
    export STATUS_MSG="This region is not supported for MAS deployment. Please try deployment in the supported regions."
    ...
    export STATUS_MSG="$DBProvisionedVPCId is not found in region $DEPLOY_REGION"
      ...
      export STATUS_MSG="OCP Cluster creation completed successfully & exiting."
  fi
