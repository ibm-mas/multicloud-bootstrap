...
NEW_VPC_ID=$(cat $GIT_REPO_HOME/aws/ocp-terraform/terraform.tfvars|grep vpc_id |cut -d "=" -f2)
NEW_VPC_PUBLIC_SUBNET_ID=$(cat $GIT_REPO_HOME/aws/ocp-terraform/terraform.tfvars|grep worker_subnet1_id|cut -d "=" -f2)
